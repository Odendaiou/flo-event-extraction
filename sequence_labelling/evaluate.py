...
def trigger_scores(threads, desc: str = "", *, verbose: bool = False, return_threads: bool = False):
	...
	for _, email in  tqdm(enumerate(threads), total = len(threads), desc="Evaluating end2end...", disable=not verbose):
				...
				if(matched_trigger):
					...
					if(gold_triggers[best_trigger_index]["type"] == predicted_trigger["type"]):
						...
						for args in predicted_arguments:
							...
							arg_role_type = re.sub(r"_\d+","",arg_role_type)

							if(best_arg_index!=-1):
								gold_arguments[best_arg_index]['type'] = re.sub(r"_\d+", "", gold_arguments[best_arg_index]['type'])
	...
	result = {
		"EM_trigger_id_scores": total_trigger_identification_scores,
		"EM_trigger_class_scores": total_trigger_class_scores,
		"EM_arg_id_scores": total_argument_identification_scores,
		"EM_arg_class_scores": total_argument_class_scores,
	}
	if return_threads:
