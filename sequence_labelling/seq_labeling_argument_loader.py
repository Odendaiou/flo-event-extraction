...
class seq_tagger_arg_multi_class(Dataset):
    ...
    def __getitem__(self, index):
        ...
        truncation=True,          # これを追加
        max_length=self.max_len   # これを追加 (512)
    )
        ...
        item['trigger_span'] = torch.tensor(self.tokenizer(event_trigger['words'], padding='max_length', truncation=True, max_length=512)['input_ids'])
        ...
        item["file_name"] = torch.tensor(self.tokenizer(file_name, padding='max_length', max_length = self.max_len, truncation=True)['input_ids'])
        #print(item["meta_srs"])
        #print(all_lengths)
